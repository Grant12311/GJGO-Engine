...
namespace GJGO
{
    namespace Renderer
    {
        inline Druid::Shader* defaultSpriteShader;

        void begin2D(Druid::Shader* const a_shader, const Camera2D &a_camera, const unsigned int a_width, const unsigned int a_height);
        void begin2D(Druid::Shader* const a_shader, const Camera2D &a_camera, const unsigned int a_width, const unsigned int a_height);

        void drawQuad(const Position2D &a_position, const Size2D &a_size, const float a_rotation = 0.0f, const Color4 a_color = {1.0f, 1.0f, 1.0f, 1.0f}, const unsigned int a_texID = 0);
